...
task PreparePackage -depends Test {
	...
	cp ..\targets\emvelope\emvelope.* ..\targets\packages\lib\net45
}
...
task Test -depends Clean,CopyTools,Compile { 
  # run specs
  ..\src\packages\Machine.Specifications.0.5.16\tools\mspec-clr4.exe .\..\targets\specs\emvelope.specs.dll
...
task Compile -depends Clean,UpdateAssemblyInfo { 
  ...
  Push-Location ..\src
  $build = $msbuild + ' "Emvelope.sln" ' + $options + " /t:Build"
